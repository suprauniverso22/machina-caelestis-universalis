...
describe('Vistas coordinadas',()=>{
 ...
 it('cambia altitud estelar con hora y lugar, conserva reversibilidad',()=>{const a=skyAt(defaults),b=skyAt({...defaults,jd:defaults.jd+.25}),c=skyAt({...defaults,latitude:-33.87,longitude:151.21});const star=(s:typeof a)=>s.stars.find(x=>x.name==='Sirius')!;expect(a.stars.length).toBe(925);expect(Math.abs(star(a).alt-star(b).alt)).toBeGreaterThan(10);expect(Math.abs(star(a).alt-star(c).alt)).toBeGreaterThan(10);expect(star(skyAt(defaults))).toEqual(star(a));expect(projectSky(90,0)).toEqual({x:320,y:310});});
